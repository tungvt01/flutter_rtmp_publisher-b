...
class CameraController extends ValueNotifier<CameraValue> {
  ...
  addTextOverlay({String text = '', int size = 0, int color = 0}) async {
    try {
      await _channel.invokeMethod<void>('addTextOverlay', <String, dynamic>{
        'text': text,
        'size': size,
        'color': color,
      });
  ...
  addImageOverlay(List<int> image, int scale) async {
    try {
      // print('**********************done: $image ************************');
      await _channel.invokeMethod<void>(
          'addImageOverlay', <String, dynamic>{'image': image, 'scale': scale});
    } on PlatformException catch (e) {
      throw CameraException(e.code, e.message ?? 'Unknown exception');
    }
  }

  addDegpegOverlay(List<int> image, int scale) async {
    try {
      // print('**********************done: $image ************************');
      await _channel.invokeMethod<void>('addDegpegOverlay',
          <String, dynamic>{'image': image, 'scale': scale});
    } on PlatformException catch (e) {
      throw CameraException(e.code, e.message ?? 'Unknown exception');
    }
  }

  addCtaOverlay(List<int> image, int scale) async {
    try {
      // print('**********************done: $image ************************');
      await _channel.invokeMethod<void>(
          'addCtaOverlay', <String, dynamic>{'image': image, 'scale': scale});
    } on PlatformException catch (e) {
      throw CameraException(e.code, e.message ?? 'Unknown exception');
    }
  }
  ...
  ///
  /// Throws a [CameraException] if the capture fails.
  Future<void> startVideoStreaming(url,
      {int bitrate = 1200 * 1024, required bool androidUseOpenGL}) async {
    if (!value.isInitialized || _isDisposed) {
      throw CameraException(
        'Uninitialized CameraController',
