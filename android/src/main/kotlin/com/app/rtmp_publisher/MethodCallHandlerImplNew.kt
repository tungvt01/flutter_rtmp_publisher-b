...
class MethodCallHandlerImplNew(
    ...
    override fun onMethodCall(call: MethodCall, result: MethodChannel.Result) {
        when (call.method) {
            ...
            "addTextOverlay" -> {
                getCameraView()?.addTextToStream(call.argument("text")!!,call.argument("size")!!,call.argument("color")!!)
            }
            "addDegpegOverlay" -> {
                println("*******************addDegPegOverlay*********************")
                println(0)
                var imgData: ByteArray = call.argument("image")!!
//                var data= lst as ByteArray

                getCameraView()?.addDegpegToStream(imgData,call.argument("scale")!!)
            }
            "addCtaOverlay" -> {
                println("*******************addDegPegOverlay*********************")
                println(0)
...
//                var data= lst as ByteArray
                ...
                getCameraView()?.addCtaImageToStream(imgData,call.argument("scale")!!)
            }
            "addImageOverlay" -> {
                println("*******************addImageOverlay*********************")
                println(0)
                var imgData: ByteArray = call.argument("image")!!
//                var data= lst as ByteArray
                println(imgData[0])
                getCameraView()?.addImageToStream(imgData,call.argument("scale")!!)
            }
